...
class LmPhone:
    ...
    def importVcfTag(iContact, iTag, iParams, iVal):
        ...
        if iTag == 'N':
            ...
            if len(s) > 1:
                ...
                iContact['name']  = s[0].strip()
                iContact['firstname'] = ''
            iContact['formattedName'] = EditContactDialog.compute_formatted_name(iContact['name'], iContact['firstname'])
    ...
    def addContactDialog(self, iDefaultContactData):
        dialog = EditContactDialog(False, iDefaultContactData, self)
        if dialog.exec():
            aContact = dialog.get_contact()
    ...
    def editContactDialog(self, iLine):
        ...
        # Edit dialog
        dialog = EditContactDialog(True, aContact, self)
        if dialog.exec():
            aContact = dialog.get_contact()
